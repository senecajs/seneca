    ...
    return {
        ...
        options: (spec) => {
            ...
            if (null == defaults_values ||
                ...
                // forward compat with Gubu defaults
                let hasGubu = false;
                for (let pn in defaults_values) {
                    let pv = defaults_values[pn];
                    if (pv.gubu || (pv.$ && pv.$.gubu$) ||
                        (pv instanceof Function && [
                            'Array',
                            'BigInt',
                            'Boolean',
                            'Function',
                ...
                let useJoi = !hasGubu &&
                    (so.legacy.options || Joi.isSchema(defaults_values, { legacy: true }));
                if (!useJoi) {
                    // TODO: use Gubu.isShape
                    let isShape = hasGubu && defaults_values.gubu && defaults_values.gubu.gubu$;
...
function make_intern() {
    return {
        ...
        prepare_spec: function (Joi, spec, opts, ctxt) {
            ...
            let joi = intern.walk(Joi, joiobj, spec, '', opts, ctxt, function (valspec) {
