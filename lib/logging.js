...
function build_log_spec(self) {
  ...
  //var es; try { throw new Error() } catch(e) { es = e.stack };
  //console.log('ORIG LOGSPEC', {v:orig_logspec}, es)
...
function build_log(self) {
  ...
  self.log = function log(entry) {
    ...
    if (instance.private$.act) {
      ...
      for (
        var i = 0;
        i < (instance.private$.act.meta.parents || []).length;
        i++
      ) {
        entry.idpath +=
          '.' + instance.private$.act.meta.parents[i][1].split('/')[0]
      }
    ...
    // console.log('LOG', level_match, entry)
