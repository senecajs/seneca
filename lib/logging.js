...
function build_log_spec(self) {
  ...
  var logspec = Common.deep(
    { text_level:{} },
    'object' === orig_logspec ? orig_logspec : default_logspec)

  // Define reverse lookup of log level values by name
  Object.keys(logspec.level_text).forEach(val => {
    var intval = logspec.text_level[logspec.level_text[val]] = parseInt(val,10)
  })
  ...
  if ('string' === typeof orig_logspec) {
    ...
    //console.log('BLS ', orig_logspec, logspec.level)
  }
...
function build_log(self) {
  ...
  self.log = function log(entry) {
    var logspec = this.private$.logspec
    ...
    entry.seneca_id = entry.seneca_id || this.id

    if(this.did) {
      entry.seneca_did = entry.seneca_did || this.did
    }

    if(this.fixedargs.plugin$) {
      entry.plugin_name = entry.plugin_name || this.fixedargs.plugin$.name
      entry.plugin_tag = entry.plugin_tag || this.fixedargs.plugin$.tag
    }

    if(this.private$.act) {
      entry.actid = entry.actid || this.private$.act.meta.id
      entry.pattern = entry.pattern || this.private$.act.def.pattern
    }
    ...
    if(level_match) {
      this.private$.logger.call(this, entry)
    }
