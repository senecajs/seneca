...
var makelogrouter = exports.makelogrouter = function( logspec ) {
  var map = null === logspec ? [] : [{level:'info+',handler:'print'}]

  if( _.isString( logspec ) ) {
    map = [logspec]
  }
  else if( logspec && logspec.map ) {
    map = logspec.map
  }
...
function shortcut( spec ) {
  ...
  else if( spec && (spec.quiet || 'quiet'===spec || spec.silent || 'silent'==spec) ) {
    return null;
  }
  else return void 0;
}
...
handlers.stream = function stream(stream,opts) {
  opts = opts||{}
  return function() {
    var args = common.arrayify(arguments)
...
handlers.emitter = function emitter(emitter) {
  return function() {
    var args = common.arrayify(arguments)
...
exports.parse_command_line = function parse_command_line( spec, logmaps ) {
  var shortentry = shortcut(spec)
  if( null === shortentry ) return;
