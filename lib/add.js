...
exports.task = exports.api_add = void 0;
...
const task = {
    ...
    flags(spec) {
        ...
        if (opts.legacy.actdef) {
            actdef.args = (0, common_1.deep)(pat);
        }
    ...
    action(spec) {
        ...
        const private$ = spec.ctx.private;
        var action_name = null == action.name || '' === action.name ? 'action' : action.name;
    ...
    rules(spec) {
        ...
        if (!opts.legacy.rules && 0 < prN) {
            actdef.gubu = (0, gubu_1.Gubu)(pattern_rules);
        }
