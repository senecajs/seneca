...
function make_delegate( instance, plugin, options ) {
  ...
  sd.__update_plugin__ = function( plugin_meta ) {
    ...
    _.each(actmetalist,function(actmeta){
      actmeta.plugin_name     = plugin_meta.name || actmeta.plugin_name || '-'
      actmeta.plugin_tag      = plugin_meta.tag || actmeta.plugin_tag || '-'
      actmeta.plugin_fullname = 
        plugin_meta.fullname || actmeta.plugin_fullname || '-'
    })
  }
...
function ERRMSGMAP() {
  return {
    unsupported_legacy_plugin: 'The plugin <%=name%> uses an unsupported legacy callback to indicate plugin definition is complete: <%=init_func_sig%> ... }. The correct format is: function(options) { ... }. For more details, please see http://senecajs.org/write-a-plugin.html',
  }
}
