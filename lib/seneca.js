...
function make_seneca($, initial_options ) {
  ...
  // Resolve options.
  var optioner = makeoptioner( optimist, initial_options.module || module.parent || module )
  var so       = optioner.set( initial_options )




  // For testing the command line specification of options
  // using, for example: --seneca.options.testmark=foo
  if( so.testmark ) {
    console.log( 'TESTMARK: '+so.testmark)
  }
  ...
  // TODO: test case where name is a valid js but not a plugin - seems to just terminate!!!
  ...
  function api_options( options ) {
    return $.exports.options = (null == options) ? optioner.get() : optioner.set( options );
  }
...
function init(so) {
  ...
  seneca.add({role:'options',cmd:'get'},function(args,cb){
