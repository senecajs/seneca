...
function Seneca($, opts) {
  ...
  self.eyes = function() {
    require('eyes').inspect(JSON.parse($.actrouter.toString()))
  }
  ...
  self.err = function( errfn, winfn ) {
    ...
    if( !_.isFunction(winfn) ) throw self.fail({code:'seneca/error_handler_no_win_function'})
    return function(err) {
      if( err  ) {
        return errfn(err)
      }
  ...
  self.register = function( plugin, pluginopts, cb ) {
    ...
    return paramcheck.register.validate(plugin,function(){
      ...
      plugin.init(sd,opts,function(err){
        ...
        var service = plugin.service && plugin.service()
        if( service ) {
          service.log = sd.log
  ...
  */
  ...
  self.use = function( arg0, arg1, arg2 ) {
    ...
    var plugindesc = _.extend({},plugin_opts)
