...
function Seneca($, opts) {
  ...
  self.register = function( plugin, cbfunc ) {
    ...
    function do_register(opts) {
      ...
      plugin.init.call(sd,sd,opts,function(err,meta){
        if( err ) {
  ...
  self.http = function( spec ) {
    ...
    function defaulthandler(req,res,args,act,responder) {
      ...
      act(args,function(){
        ...
        responder.apply(null,responder_args)
      })
    }
  ...
  function do_act(instance,actfunc,isparent,args,cb){
    ...
    function perform(actfunc) {
      ...
      self.log.debug('act',function(){
        ...
        return [plugin_nameref[0]||'-',plugin_nameref[1]||'-',actid,'IN',argsdesc]
      })
      ...
      try {
        ...
        delegate.log = function() {
          ...
          if( _.isFunction(actfunc.log) ) {
            actfunc.log.apply(instance,[args[0]].concat(actid).concat(args.slice(1)))
          }
          else {
        ...
        if( actfunc.parent ) {
          delegate.parent = function(args,cb) {
            //console.log('parent:'+delegate)
            do_act(delegate,actfunc.parent,true,args,cb)
          }
        }


        //console.log('actfunc.call '+delegate)
        actfunc.call(delegate,args,function(err){
          var args = arrayify(arguments)
          ...
          if( err ) {
            ...
            self.log.debug('act',function(){
              ...
              var resout = _.flatten([plugin_nameref[0]||'-',plugin_nameref[1]||'-',actid,'OUT',resdesc], true)
              return resout
            })
  ...
  self.act = function(args,cb) {
    ...
    var actfunc = this.findact(args)

    if( !actfunc ) {
      if( _.isUndefined(args.default$) ) {
        this.fail({code:'seneca/act_not_found',args:args},cb)
      }
      else return cb(null,args.default$);
    }
    else do_act(this,actfunc,false,args,cb)
  }
