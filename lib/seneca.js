...
function Seneca($, opts) {
  ...
  self.register = function( plugin, cbfunc ) {
    ...
    function do_register(opts) {
      ...
      function install_plugin(err,meta){
        ...
        self.log.info('register','install',pluginref,{service:!!service},fullname!=pluginref?fullname:undefined)

        $.ready_queue.push({cb:function(done){

          self.act({role:'options',cmd:'get',base:fullname,default$:{}},self.err(done,function(fullname_options){
            var shortname = fullname != plugin.name ? plugin.name : null
            if( !shortname && 0 == fullname.indexOf('seneca-') ) {
              shortname = fullname.substring('seneca-'.length)
            }
            ...
            if( shortname ) {
              self.act({role:'options',cmd:'get',base:shortname,default$:{}},self.err(done,function(name_options){
                var opts = _.extend({},name_options,fullname_options,plugin.opts||{})
                do_init(opts)
              }))
            }
            else return do_init( _.extend({},fullname_options,plugin.opts||{}) )
          }))


          function do_init(opts) {
            self.act({init:plugin.name,tag:plugin.tag,options:opts,default$:{}},function(err,out){
              if( !err ) {
                self.log.info('register','ready',pluginref,out)
              }
              done(err)
            })
          }
      ...
      var args = [opts,install_plugin]

      // handle legacy plugins with function(seneca,opts,cb)
      if( 3 == plugin.init.length ){
        args.unshift(sd)
      }


      // handle legacy plugins that are async on setup
      var meta = plugin.init.apply(sd,args)
      meta = (plugin.init.length <= 1 && void 0 == meta) ? {} : meta

      if( meta ) {
        install_plugin(null,meta)
      }
    }
  }
  ...
  self.http = function( spec ) {
    ...
    function defaultresponder(req,res,handlerspec,err,obj) {
      ...
      var code   = err ? (err.seneca ?  err.seneca.status : 500) : 200;
