...
function make_seneca($, opts) {
  ...
  function handle_error_args( args, ctxt ) {
    ...
    code = _.isError(first) && _.isString(first.code) ? first.code : code
    code = _.isObject(first) && _.isString(first.code) ? first.code : code 


    if( _.isObject(first) && !_.isError(first) ) {
      valstart = 0
    }
  ...
  self.add = function() {
    ...
    if( _.isString( args) ) {
      ...
      try {
        ...
        throw self.fail('add_string_pattern_syntax',{argstr:args,syntax:e.message,line:e.line,col:col})
      }
      ...
      // TODO: common.owndesc not suitable for listing of arguments - need a util for this
      else throw self.fail('add_pattern_object_expected_after_string_pattern',{args:common.owndesc(arguments,1)})
    }
    else if( !_.isObject(args) ) {
      throw self.fail('add_pattern_object_expected',{args:common.owndesc(arguments,1)})
    }
    ...
    if( _.isFunction(paramspec) ) {
      ...
      try {
        ...
        throw new self.fail('bad-param-spec',{message:pe.message})
      }
    ...
    if( !_.isFunction(actfunc) ) {
      throw self.fail('add_action_function_expected',{args:common.owndesc(arguments,1)})
    }
    ...
    if( void 0 == actmeta ) {
      ...
      throw self.fail('add_action_metadata_not_an_object',{actmeta:common.owndesc(arguments,1)})
    }
  ...
  // Execute the action only if the first argument is true.
  ...
  self.act_if = function() {
    ...
    else throw this.fail('act_if_expects_boolean',{first:first}) 
  }
  ...
  self.declare = function( arg0, arg1, arg2 ) {
    var self = this
    ...
    function perform(actmeta) {
      ...
      try {
        ...
        actmeta.func.call(delegate,callargs,function(err){
          ...
          // for errors thrown inside the callback
            ...
            if( !opts.test.silent) {
              self.log.error('act','err',actid, 'callback', error.message, actmeta.id, stackfirst(error) )
            }

            var err = instance.fail( descerror({code:'seneca/act_callback_error',args:args},error) )
      ...
      // for errors inside the action
        ...
        if( !ex.seneca ) {
          ...
          // sent to cakkback as inside action
  ...
  function handle_act_args(orig) {
    ...
    if( _.isString(orig[0]) ) {
      ...
      var argsobj = _.isObject(orig[0]) && !_.isArray(orig[0]) && !_.isFunction(orig[0]) ? orig[1] : {}
      ...
      try {
        ...
        var col = 1==e.line?e.column-1:e.column
        throw self.fail('act_string_args_syntax',{argstr:orig[0],syntax:e.message,line:e.line,col:col})
      }
    }
    else {
      cb = _.isFunction(orig[1]) ? orig[1] : noop

      var argsobj = _.isObject(orig[0]) && !_.isArray(orig[0]) && !_.isFunction(orig[0]) ? orig[0] : null
      if( !argsobj ) {
        throw self.fail('act_no_args',{args:orig})
      }

      args = _.extend({},argsobj)
    }
  ...
  self.delegate = function(fixedargs) {
    ...
    delegate.toString = function(){
      ...
      return strdesc = self.toString()+(_.keys(vfa).length?'/'+common.owndesc(vfa,0,false):'')
    }
