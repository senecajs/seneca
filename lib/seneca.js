...
function Seneca($, opts) {
  ...
  if( argv.seneca ) {
    if( argv.seneca.log ) {
      ...
      if( argv.seneca.log.quiet ) {
        // empty map does the work!
      }
      else if( argv.seneca.log.print ) {
        opts.log.map.push({level:'all',handler:'print'})        
      }
      else {
        // parse: level=,type=,plugin=,tag=,handler=
        // handler can be print,file:path

        var keys = {level:1,type:1,plugin:1,tag:1,handler:1}
        var entry = {}
        var parts = (''+argv.seneca.log).split(',')
        _.each(parts,function(part){
          ...
          if( 2 <= kv.length ) {
            ...
            if( 'handler' == key ) {
              ...
              var m
              if( 'print' == handler ) {
                entry[key]=logging.handlers.print
              }
              else if( m = /^file:(\/\/)?(.*)$/.exec(handler) ) {
                entry[key]=logging.handlers.file(m[2])
              }
            }
            else if( keys[key] ) {
              entry[key]=kv[1]
        ...
        if( 0 < _.keys(entry).length && keys.handler ) {
          opts.log.map.push(entry)
        }
      }
    }
  }
  ...
  self.register = function( plugin, pluginopts, cb ) {
    ...
    function do_register(opts) {
      self.log.info('register','init',plugin.name,plugin.tag)
      ...
      plugin.init(sd,opts,function(err,servicefunc){
        ...
        self.log.info('register','ready',plugin.name,plugin.tag,{service:!!service})
        $.popready(pluginref)
        cbfunc(null)
      })
    }
  ...
  function resolveplugin( pluginname, tag ) {
    ...
    if( plugin ) {
      ...
      if( plugin_module ) {
        ...
        seneca.log.info('plugin','notfound',pluginname)
      }
    }
