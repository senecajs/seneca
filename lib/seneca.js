...
function make_seneca($, so) {
  ...
  so.log = so.log || so.logger || so.logging || {}
  ...
  // legacy api
  if( 'print'==so.log ) {
    so.log = {map:[{level:'all',handler:'print'}]}
  }
  ...
  function api_register( plugin, cbfunc ) {
    ...
    return do_register( plugin.options )
    ...
    function do_register(opts) {
      ...
      function install_plugin(err,meta){
        ...
        if( !plugin.declare ) {
          ...
          self.act({init:plugin.name,tag:plugin.tag,default$:{},gate$:true,proxy$:false},function(err,out){
            if( err ) {
  ...
  function handle_error_args( args, ctxt ) {
    ...
    var message = (so.message[ctxt.plugin] && so.message[ctxt.plugin][code])
  ...
  function api_client() {
    ...
    */
    ...
    //$.ready_queue.push({cb:function(done){
    self.act('role:transport,cmd:client',{config:config,gate$:true},function(err,out){
    ...
    client.findact = function( args ) {
      ...
      actmeta = {
        func: function(args,done){ send.call(client,args,done) },
        plugin_nameref:'-',
        log:client.log,
