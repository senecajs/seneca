...
//transport client pins should not need declare
//create a patrun for these pins
...
function make_seneca($, initial_options ) {
  ...
  var optioner = makeoptioner(optimist)
  ...
  if( process.env.SENECA_LOG ) {
    so.log.map = so.log.map || []
    var loggingconf = process.env.SENECA_LOG
    logging.parse_command_line( loggingconf, so.log.map )
  }

  if( argv.seneca ) {
    if( argv.seneca.log ) {
      so.log.map = so.log.map || []
      logging.parse_command_line( argv.seneca.log, so.log.map )
    }
  }
  ...
  self.on('error',common.noop) // prevent process exit
    ...
    return function(err) {
      if( err  ) {
        ...
        if( _.isFunction(winfn) ) {
          winfn.apply(null,arr(arguments,1))
        }
      }
    }
  }
  ...
  function api_register( plugin, cbfunc ) {
    ...
    // TOOD: encapsulate plugin delegate construction
    ...
    function do_register() {
        ...
        self.act({role:'options',cmd:'get',base:fullname,default$:{}},function(err,fullname_options){
          ...
          if( shortname ) {
            self.act({role:'options',cmd:'get',base:shortname,default$:{}},function(err,name_options){
              ...
              do_init( _.extend({},name_options,fullname_options,plugin.options||{}) )
            })
          }
          else do_init( _.extend({},fullname_options,plugin.options||{}) )
        })
      }
      else {
        return do_init( plugin.options || {} )
      }
       */
      ...
      function resolve_options( fullname, plugin ) {
        ...
        //console.log('resolveopts',fullname,shortname,outopts,so)
      ...
      function install_plugin(err,meta){
        ...
        if( void 0 != meta.export ) {
          ...
          // options export is special, you can't override options passed in seneca constructor
          /*
          if( 'options' == plugin.name ) {
            $.exports[plugin.name] = _.extend({},meta.export,so[plugin.name],so.plugin[plugin.name])
          }
          else {
          ...
          //console.log('PLUGIN',pluginref,meta.export)

          $.exports[pluginref] = meta.export
          exports.push(pluginref)

          // update seneca options
          if( 'options' == pluginref ) {
            so = _.extend({},meta.export,so)
          }
        }
    ...
    function perform(actmeta) {
      ...
      if( do_log ) {
        self.log.debug('act',plugin_nameref[0]||'-',plugin_nameref[1]||'-',actid,'IN',function(){
          return [actmeta.descdata ? actmeta.descdata(args) : common.descdata(args), actmeta.id]
        })
      }
      

      var delegate = instance.delegate({actid$:actid,ignoregate$:!!args.gate$})
      ...
      var act_done = function(err) {
        ...
        if( err ) {
          ...
          if( do_log ) {
            self.log.debug('act',plugin_nameref[0]||'-',plugin_nameref[1]||'-',actid,'OUT',function(){
              return _.flatten( [ _.flatten([ actmeta.descdata ? actmeta.descdata(args.slice(1)) : common.descdata(args.slice(1)) ], true), actmeta.id ] )
            })
          }
        ...
        // for errors thrown inside the callback
          ...
          if( !so.test.silent) {
            self.log.error('act','err',actid, 'callback', err.message, actmeta.id, error.stack )
          }
      ...
      $.executor.execute({
        id:      actid,
        gate:    !!callargs.gate$,
        ignoregate: !!callargs.ignoregate$,
...
function init(so) {
  ...
  seneca.add({role:'seneca',ready:true},function(args,done){
    seneca.emit('ready')
    done()
  })
