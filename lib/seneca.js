  ...
  self.register = function( plugin, cbfunc ) {
    ...
    function do_register(opts) {
      ...
      function install_plugin(err,meta){
        ...
        self.log.debug('register','install',pluginref,{service:!!service},fullname!=pluginref?fullname:undefined)
  ...
  self.http = function( spec ) {
    ...
    function makedispatch(act,urlspec,handlerspec) {
      return function( req, res, next ) {
        ...
        var preware = spec.preware || function(){arguments[2]()}

        preware.call(si,req,res,function(err){
          if(err ) return next(err);
    ...
    var maprouter = httprouter(function(http){
      ...
      // lastly, try preware by itself against prefix if nothing else matches
      // needed for common auth checks etc
      // ensures preware is always called
      if( spec.preware ) {
        http.get(prefix, spec.preware)
      }
    })
