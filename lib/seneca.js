...
function Seneca($, opts) {
  ...
  self.fail = function( meta, cb ) {
    ...
    error && ( err_seneca.error = error )

    delete err_seneca.msg
    delete err_seneca.message

    msg = "Seneca: " + _.template( msg, _.extend({json$:JSON.stringify},err_seneca) )

    //console.log('fail-err_s:'+JSON.stringify(err_seneca))
    ...
    if( !actfunc ) {
      ...
      try {
        actfunc(args,function(err){
          ...
          if( err ) {
            log('act','err',tag,err.message,err.stack&&(err.stack+' \n ').split('\n')[1] )
        ...
        log('act','err',tag, error.message, error.stack&&(error.stack+' \n ').split('\n')[1] )
        throw self.fail({code:'seneca/act_error',error:error,args:args})
      }
    }
  }
...
function init(opts,extcb) {
  ...
  initplugins(seneca,function(err){
    ...
    try {
      ...
      seneca.log('init','err',e.message,e.stack&&(e.stack+' \n ').split('\n')[1] )
      throw e
    }
  })
      ...
      if( pI < plugins.length ) {
        try {
          ...
          if( e.seneca && e.seneca.external ) {
            ...
            //console.log('PINIT')
            //eyes.inspect(e)
            seneca.fail({code:'seneca/plugin_exception',error:e,pluginname:pluginname},extcb)
          }
        }
      }
        ...
        try {
          ...
          if( e.seneca ) {
            ...
            throw seneca.fail({code:'seneca/callback_exception',error:e,pluginname:pluginname,external:true})
          }
        }
      }
    }
