...
function make_seneca($, opts) {
  ...
  //setInterval(function(){
  //  _.each( $.statwindows, function(w) { w.tick() } )
  //}, opts.timestat_interval )
  ...
  self.add = function() {
    ...
    //delete $.statwindows[actmeta.argpattern]
    //actmeta.statwindow =
    //  $.statwindows[actmeta.argpattern] =
    //  new stats.NamedStats( opts.stats.count, )
    //eep.EventWorld.make().windows().monotonic(eep.Stats.all,new eep.WallClock(opts.timestat_interval))

    //actmeta.statwindow.on('emit',function(timestats){
    //  for( var stat in timestats ) {
    //    actstats.t[stat] = timestats[stat]
    //  }
    //})
  ...
  function do_act(instance,actmeta,isprior,origargs,cb){
    ...
    function perform(actmeta) {
      ...
      try {
        ...
        $.stats.act.failed++
        actstats.f++
        //actmeta.statwindow.enqueue( actend-actstart )
...
function init(opts) {
  ...
  seneca.add({role:'seneca',stats:true}, function( args, done ) {
    ...
    if( args.pattern && $.stats.actmap[args.pattern] ) {
      ...
      stats.t = $.timestats.calculate(args.pattern)
    }
    else {
