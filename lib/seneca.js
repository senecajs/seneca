...
function Seneca($, opts) {
  ...
  self.act = function(args,cb) {
    ...
    function perform() {
      ...
      try {
        self.emit('act-in', actfunc.argpattern, actid, args)

        actfunc(args,function(err){
          var args = arrayify(arguments)

          if( err ) {
            self.log.error('act','err',actid,err.message,stackfirst(err) )
            ...
            self.log.debug('act',function(){
              ...
              var resout = _.flatten(['out',actid,resdesc], true)
              return resout
            })
