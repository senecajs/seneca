...
function make_seneca($, opts) {
  ...
  self.register = function( plugin, cbfunc ) {
    ...
    sd.fail = makefail.call(sd,{type:'plugin'})
  ...
  function makefail( ctxt ) {
    ...
    var fail = function() {
      ...
      var code = 
            _.isString(args[0]) ? args[0] : 
            _.isError(args[0]) ? (_.isString(args[0].code) ? args[0].code : 'unknown') : 
          'unknown'

      var valstart = 1
      var error    = _.isError(args[0]) ? args[0] : _.isError(args[1]) ? (valstart=2,args[0]) : null
      ...
      var message  = (error && _.isString(error.message) && error.message) || 'Unknown error.'
      var logargs  = [ctxt.type, ctxt.plugin, ctxt.tag, ctxt.id, code, message, error].concat( args.slice(valstart) )
      ...
      function die( err ) {
        if( null == error ? true : (!error.stayalive ) ) {
  ...
  self.fail = makefail.call(self,{type:'seneca'})
