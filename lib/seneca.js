    ...
    function loadplugin( pluginname ) {
      ...
      if( !plugin ) {
        ...
        // try to load as a built-in module
          ...
          plugin_module = require('./plugin/'+pluginname+'.js')
        }
    ...
    function eachplugin(pI) {
      if( pI < plugins.length ) {
        try {
          ...
          if( plugin ) {
            seneca.log('plugin','init',pluginname)
            plugin.init(seneca,pluginopts,function(err){
              err ? cb(err) : eachplugin(pI+1)
            })
          }
          else {
...
module.exports.register = function( plugin ) {
  ...
  if( !plugin.name ) throw new Error('seneca: plugin argument has no name property')
  if( !plugin.plugin ) throw new Error('seneca: plugin argument has no plugin function')
  if( !plugin.init ) throw new Error('seneca: plugin argument has no init function')
