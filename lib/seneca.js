...
function make_seneca($, initial_options ) {
  ...
  // Resolve options.
  var optioner = $.optioner = makeoptioner( argv, initial_options.module || module.parent || module )
  ...
  self.patrun = function() { return $.actrouter; };
  ...
  function api_client() {
    ...
    self.findact = function( args ) {
      //console.log('BAD findact '+util.inspect(config))
        ...
        func: function(args,done) { 
          ...
          function checkready() {
            ...
            if( am && am.client$ ) {
              ...
              //console.log('count '+count+' '+util.inspect(config))
              count++
    ...
    self.act( 
      ...
      function(err,sendclient) {
        ...
        //console.log('CLIENT '+self.id+' findact orig: '+self.findact.mark+' bind: '+findact.mark+' config:'+util.inspect(config))


        self.findact = function( args ) {
          //console.log('GOOD findact '+util.inspect(config))

          var clientmatch = sendclient.match.call( self, args )
          //console.log(self.id+' client-findact sendclient:'+sendclient+' ON '+util.inspect(self.util.clean(args))+' -> '+clientmatch)
  ...
  function api_findact(args) {
    //console.log('API findact '+util.inspect(args))
