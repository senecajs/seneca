...
function make_seneca(initial_options ) {
  ...
  // Add builtin actions.
  self.add( {role:'seneca',  stats:true},  action_seneca_stats )
  self.add( {role:'seneca',  ready:true},  action_seneca_ready )
  self.add( {role:'seneca',  cmd:'close'}, action_seneca_close )
  self.add( {role:'options', cmd:'get'},   action_options_get  )
  ...
  function action_seneca_stats( args, done ) {
    ...
    if( args.pattern && $.stats.actmap[args.pattern] ) {
      ...
      if( summary ) {
        ...
        _.each( $.stats.actmap, function(a,p) { 
          $.stats.actmap[p].time = $.timestats.calculate(p) 
        })
      }
    }
