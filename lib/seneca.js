...
function make_seneca($, opts) {
  ...
  self.depends = function() {
    ...
    _.each(deps, function(depname){
      if( !_.contains($.plugin_order.byname,depname) &&
        ...
        return self.fail('plugin_required',{plugin:plugin,dependency:depname})
      }
    })
  }
  ...
  function makefail( ctxt ) {
    ...
    var fail = function() {
      ...
      var code = 
            _.isString(first) ? first : 
            _.isError(first) ? (_.isString(first.code) ? first.code : 'unknown') : 
            'unknown'
      ...
      var message  = (opts.message[ctxt.plugin] && opts.message[ctxt.plugin][code]) || 
                      (error && _.isString(error.message) && error.message) || code
      var valmap = args[valstart]
      ...
      if( _.isFunction( stayalive ) ) {
        stayalive( code, valmap )
      }
      ...
      // make sure we close down within options.faildelay seconds
      if( !stayalive ) {
  ...
  self.fail = makefail.call(self,{type:'sys',plugin:'seneca'})
...
var MSGMAP = {
  seneca:{
    plugin_required:'Plugin "<%=plugin%>" depends on plugin "<%=dependency%>", which is not yet loaded.'
  }
}
