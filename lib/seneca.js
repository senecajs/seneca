...
*/
  ...
  self.api = function( pluginname ) {
    ...
    console.dir(self.rolemap)

    // TODO: doesn't smell right at all
    var plugin = core.plugins[pluginname]
    if( !plugin ) {
      plugin = self.rolemap[pluginname]

      if( !plugin ) {
        throw {err:'plugin_unknown',name:pluginname}
      }
    }
    // FIX: check for api func
  ...
  // FIX: don't like this - creates a new instance
...
module.exports.init = function(opts,cb) {
  ...
  if( 'string' == typeof(entity) || (entity && entity.store) ) {
  ...
  */
  ...
  console.log('s-1:'+seneca)


  initplugins(seneca,function(err){
    console.log('s-2:'+seneca)

    // FIX: auto load mem plugin if no entity

    // FIX: moving init inside seneca should remove need for this hacking
    try {
    var entity = new Entity(seneca)
      } catch(e) { console.log(e) }
    console.log('e-1:'+entity)

    seneca.$.entity = entity

    entity.logger$(function(){
      var args = Array.prototype.slice.call(arguments)
      args.unshift(entity.$.api$.name)
      args.unshift('entity')

      // FIX: not the log I'm looking for
      //log.apply(self,args)
      console.log.apply(null,args)
    })

    console.log('s-3:'+seneca)
  ...
  function initplugins(seneca,cb) {
    ...
    function loadplugin( pluginname ) {
      ...
      console.log('loadplugin2:'+pluginname)

      var plugin = core.plugins[pluginname]
      console.log('plugin:'+pluginname+' p='+plugin)

      if( !plugin ) {
        var plugin_module
        var builtin_path = './plugin/'+pluginname

        console.log('builtin_path: '+builtin_path)
        ...
        console.log('plugin_module:'+plugin_module)
    ...
    function eachplugin(pI) {
      if( pI < plugins.length ) {
        try {
          ...
          console.log('eachplugin:'+pluginname)
