...
*/
  ...
  self.add = function() {
    ...
    var args = parse_pattern(self,arguments,'paramspec:o? action:f actmeta:o?')

    /*
    var args = norma( '{strargs:s? objargs:o? paramspec:o? action:f actmeta:o?}', arguments )
    
    try {
      var argsobj = _.extend(
        {}, 
      ...
      throw self.fail({code:'seneca/string-args-syntax-error',argstr:args.strargs,args:args.objargs})
    }
     */
...
*/
  ...
  function handle_act_args(self,orig) {
    ...
    /*
    var args = norma('{strargs:s? objargs:o? done:f?}',orig)
  ...
  function parse_pattern(instance,args,normaspec,fixed) {
    var args = norma('{strargs:s? objargs:o? '+(normaspec||'')+'}', args)
