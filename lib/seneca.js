...
function resolve_plugin( plugindesc, seneca, opts ) {
  seneca.log.debug('plugin','resolve',plugindesc)
  ...
  if( !plugindesc.init) {
    ...
    try {
      ...
      seneca.log.debug('plugin','require',builtin_path,fullname)
      ...
      if( e.message && -1 != e.message.indexOf("'"+builtin_path+"'") ) {
        ...
        seneca.log.info('plugin','require',fullname)

        var plugin_names = [name,'seneca-'+name]
...
function Seneca($, opts) {
  ...
  self.logroute = function(entry,handler){
    ...
    //console.log('logrouter='+$.logrouter)
  }
  ...
  self.register = function( plugin, cbfunc ) {
    ...
    function do_register(opts) {
      ...
      plugin.init(sd,opts,function(err,meta){
        if( err ) {
    ...
    function perform() {
      ...
      try {
        ...
        actfunc(args,function(err){
          ...
          if( err ) {
            self.log.error('act','err',tag,err.message,stackfirst(err) )
            ...
            self.log.debug('act',function(){
              ...
              var resout = _.flatten(['out',tag,resdesc], true)
              return resout
            })
        ...
        self.log.error('act','err',tag, error.message, stackfirst(error) )
  ...
  self.use = function( arg0, arg1, arg2 ) {
    ...
    if( _.isString(plugin) ) {
      ...
      plugindesc.name = plugin.name || plugin.seneca_name || 'anon-'+idgen(6)
...
/*
  function initplugin(pluginname,plugin,pluginopts,cb) {
    seneca.log.info('plugin','init',pluginname,pluginopts)
    seneca.register(plugin,pluginopts,cb)
  }


  function resolveplugin( pluginname, tag ) {
    seneca.log.info('plugin','load',pluginname,tag)

    var plugin = seneca.plugin(pluginname,tag)

    if( plugin ) {
      seneca.log.info('plugin','exists',pluginname,tag)
      return null
    }
    else {
      var plugin_module
      ...
      // try to load as a built-in module
      try {
        seneca.log.info('plugin','require',builtin_path,pluginname,tag)
        ...
        if( e.message && -1 != e.message.indexOf("'"+builtin_path+"'") ) {
          ...
          // try to load as a seneca repo module
          seneca.log.info('plugin','require',pluginname,tag)
      ...
      if( plugin_module ) {
        seneca.log.info('plugin','create',pluginname,tag)

        if( !_.isFunction(plugin_module) ) {
          throw seneca.fail({code:'seneca/plugin_not_function',pluginname:pluginname})
        }

        plugin = {
          name:plugin_func.name || plugin_func.seneca_name || 'anon-'+uuid().substring(0,8),
          init:plugin_module,
          tag:tag
        }
      }
      else {
        seneca.log.info('plugin','notfound',pluginname,tag)
      }
    }
  ...
  function eachplugin(pI) {
    if( pI < plugins.length ) {
      try {
        var plugindesc = plugins[pI]
        var pluginopts = {}
        var plugin

        if( _.isObject(plugindesc) ) {
