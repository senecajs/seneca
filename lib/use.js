...
function make_use(ordu, callpoint) {
    return function use() {
        ...
        ordu.exec(ctx, data, {
                ...
                // console.log('RES', res.err && res.err.details, res.err && res.err.seneca)
                if (res.err) {
    ...
    return {
        op: {
            seneca_plugin: (tr, ctx, data) => {
                Hoek.merge(data, tr.out.merge);
                ctx.seneca.private$.plugins[data.plugin.fullname] = tr.out.plugin;
                return { stop: false };
            },
            seneca_export: (_, ctx, data) => {
                Object.assign(ctx.seneca.private$.exports, data.exports);
                return { stop: false };
            }
        },
        args: (spec) => {
        ...
        delegate: (spec) => {
            ...
            return new Promise(resolve => {
                // seneca
                delegate.add(plugin_define_pattern, (msg, reply) => {
                    resolve({
                        op: 'merge',
                        out: { delegate, plugin_done: reply }
                    });
                });
                delegate.act({
                    role: 'seneca',
...
function define_plugin(delegate, plugin, options) {
    ...
    if (plugin.define.length > 1) {
        ...
        throw delegate.error('unsupported_legacy_plugin', plugin);
    }
    if (options.errors) {
        plugin.eraro = Eraro({
            package: 'seneca',
    ...
    var meta;
    try {
        //console.log('WWW define_plugin', plugin, options)
