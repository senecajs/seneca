...
function pins(inpin) {
    return (Array.isArray(inpin) ? inpin : [inpin])
        .reduce((a, p) => (a.push('string' === typeof p ? p.split(';').map(pp => (0, jsonic_1.default)(pp)) : p), a), [])
        .filter((n) => null != n)
        .flat();
}
...
function idnid(opts) {
    const gen = (0, nid_1.default)(opts);
    const alphabet = (opts && opts.alphabet) || '0123456789abcdefghijklmnopqrstuvwxyz';
    const letters = alphabet.split('').filter((c) => !/[0-9]/.test(c));
...
function parse_jsonic(str, code) {
    ...
    try {
        return null == str ? null : (0, jsonic_1.default)(str);
    }
    catch (e) {
        throw error(code, {
            argstr: str,
...
function pincanon(inpin) {
    if ('string' == typeof inpin) {
        return pattern((0, jsonic_1.default)(inpin));
    }
