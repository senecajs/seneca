...
function msgdesc(msg, maxlen = 111) {
    let desc = inspect(clean(msg))
        .replace(/\n/g, '');
    desc = desc.substring(0, maxlen) + (maxlen < desc.length ? '...' : '');
    return desc;
}
exports.msgdesc = msgdesc;
