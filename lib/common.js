...
History.prototype.add = function add (obj) {
  ...
  if (null != obj && null != obj.id) {
    delete this.map[this.log[this.next]]
    this.log[this.next] = obj.id
    this.map[obj.id] = obj
...
History.prototype.get = function get (id) {
  return null == id ? null : this.map[id] || null
}

History.prototype.list = function list (len) {
  Assert.ok(null == len || 0 <= len)
  len = null == len ? -1 : len

  if (0 === this.size || 0 === len ) return [];

  return (0 === this.laps ? [] : this.log.slice(this.next, this.size))
    .concat(this.log.slice(0, this.next)).slice(len < 0 ? 0 : -len)
}
