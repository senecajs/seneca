...
function internalize_msg(msg) {
  ...
  var meta = msg.meta$ || {}
  delete msg.meta$
  msg.__proto__ = { meta$: meta }

  return msg
}

function stringifyJSON(msg) {
  if (!msg) return

  var ext_msg = externalize_msg(msg)

  try {
    return JSON.stringify(ext_msg)
  } catch (e) {
    return JSS(ext_msg)
  }
}
...
function parseJSON(data) {
  ...
  try {
    return internalize_msg(JSON.parse(str))
  } catch (e) {
    e.input = str
    return e
  }
  ...
  function handle_request(req, res) {
    ...
    req.on('end', function() {
      ...
      seneca.act(msg, function(err, out) {
        ...
        console.log('LISTEN OUT', JSON.parse(spec.body))
...
function web_modify_response(spec) {
  spec.body = stringifyJSON(spec.err || spec.out)
    ...
    send: function(msg, reply) {
      ...
      config.wreck.request(spec.method, spec.url, spec.wreck, function(
        ...
        Wreck.read(res, spec.wreck.read, function(err, body) {
          console.log('CLIENT RECEIVE', err, JSON.parse(body))
          reply(err || parseJSON(body))
        })
      })
    }
  })
...
function web_modify_request(spec) {
  spec.body = stringifyJSON(spec.msg)
  spec.headers['Content-Length'] = Buffer.byteLength(spec.body)
