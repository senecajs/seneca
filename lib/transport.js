    ...
    self.ready(function () {
      ...
      sd.act(
        ...
        function (err, liveclient) {
          if (err) {
            return sd.emit('error', internals.error(err, 'transport_client', config))
          }
          if (liveclient === null) {
            return sd.emit('error', internals.error('transport_client_null',
                                          Common.clean(config)))
          }
...
internals.parse_config = function (args) {
  ...
  var config = args.filter(function (arg) {
    if (arg) {
      return arg
    }
  })
