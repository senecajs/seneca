...
function externalize_msg(seneca, msg) {
  if (!msg) return
...
function externalize_reply(seneca, out, meta) {
  if (!out) {
    out = { meta$: meta }
    meta.empty = true
  } else {
...
function internalize_reply(seneca, out) {
  var empty = false
...
function action_client(msg, reply) {
  ...
  //console.log('NGT IN',msg,msg.meta$)
  ...
  //console.log('NGT AC',client_msg,seneca.list('role:transport'))
  //console.dir(seneca.root.private$.ge.state(),{depth:null})
    ...
    send: function(msg, reply) {
      ...
      config.wreck.request(spec.method, spec.url, spec.wreck, function(
        ...
        Wreck.read(res, spec.wreck.read, function(err, body) {
          reply(internalize_reply(seneca, err || parseJSON(body)))
        })
      })
    }
  })
...
function web_modify_request(seneca, spec) {
  spec.body = stringifyJSON(externalize_msg(seneca, spec.msg))
  spec.headers['Content-Length'] = Buffer.byteLength(spec.body)
