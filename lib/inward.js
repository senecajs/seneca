...
function inward_msg_meta(ctxt, data) {
  ...
  msg.meta$.plugin = _.extend(msg.meta$.plugin, ctxt.actmeta.plugin)
  ...
  if (parent) {
    msg.meta$.parents.push(parent)
  }

  //console.log('IW MM',msg.meta$)
}


function inward_msg_modify(ctxt, data) {
  data.msg = _.extend(data.msg, _.omitBy(ctxt.seneca.fixedargs, function (v,p) {
    return '$' == p[p.length-1]
  }))
}
