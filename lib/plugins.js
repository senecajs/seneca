...
module.exports.register = function (so, callpoint) {
  return function api_register (plugin) {
    ...
    var definition = function (so, next) {
    ...
    if (!seneca.private$.ready) {
      seneca.private$.definitions.push(definition)
      return
    }
    ...
    definition(so, function (err, plugin) {
      ...
      module.exports.init(seneca, so, plugin, function (err) {
        if (err) {
          throw err
        }
      })
...
module.exports.init = function (seneca, so, plugin, next) {
  seneca.act({
    init: plugin.name,
    tag: plugin.tag,
    default$: {},
