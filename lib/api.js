...
exports.delegate = function (fixedargs, fixedmeta) {
  ...
  function delegate_toString() {
    ...
    strdesc =
      self.toString() + (Object.keys(vfa).length ? '/' + Jsonic.stringify(vfa) : '')
...
exports.list = function (pattern) {
  return this.private$.actrouter.list(null == pattern ? {} : Jsonic(pattern))
    .map(x=>x.match)
...
exports.client = function (callpoint) {
  return function api_client() {
    ...
    var pins =
      config.pins || (Array.isArray(config.pin) ? config.pin : [config.pin || ''])
