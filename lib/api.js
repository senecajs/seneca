...
// TODO: old prior as separate func
...
exports.prior = function () {
  ...
  if (priordef) {
    // console.log('PRIOR', arguments)
    // console.log(priordef)
    // console.log(priordef.func.toString())

    msg.prior$ = priordef.id
    // this.act(msg, reply)

    const meta = new Act.intern.Meta(this, this.options(), msg, reply)
    const delegate = Act.intern.make_act_delegate(this, this.options(), meta, priordef)
    let ctxt = { actdef: priordef }
    let data = { meta, msg }
    Inward.msg_modify(ctxt, data)
    
    const priorreply = function(err, out) {
      let haserr = Util.isError(err)
      return reply.call(delegate, haserr?err:null, out || (!haserr && err), meta)
    }
    priorreply.seneca = delegate
    
    // console.log('CALL', priordef.func.name, msg, reply, meta)

    return priordef.func.call(delegate, msg, priorreply, meta)
  } else {
    var meta = msg.meta$ || {}
