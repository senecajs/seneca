...
function MySQLStore() {
  ...
  self.save$ = function(args,cb){
    ...
    if ( update ) {
      ...
      self.connection.query( query, entp, function( err, result ) {
        if ( err ){
          seneca.log( args.tag$,'save/update query',query, entp )
      ...
      self.connection.query( query, entp, function( err, result ) {
        if ( err ){
          seneca.log( args.tag$,'save/insert query',query, entp )
  ...
  self.load$ = function( args, cb ){
    ...
    self.connection.query( query, function( err, res ){
      if ( err ) {
        seneca.log( args.tag$,'load query',query )
        seneca.fail( {code:'load',store:self.name,error:err},cb )
      }
      else{
  ...
  self.list$ = function( args, cb ){
    ...
    self.connection.query( query, function( err, results ){
      if ( err ) {
        seneca.log( args.tag$,'list query',query )
        seneca.fail( {code:'list',store:self.name,error:err},cb )
      }
      else{
        var list = []
  ...
  self.remove$ = function(args,cb){
    ...
    self.connection.query( query, function( err, result ) {
      if ( err ){
        seneca.log( args.tag$,'sremove query',query )
        seneca.fail( {code:'remove',store:self.name,error:err},cb )
      }
      else{
        cb( null, result )
      }
