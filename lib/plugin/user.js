    ...
    return function(req,res){
      ...
      var redirecturl = 
        clients[service.name].getAuthorizeUrl(
            {redirect_uri:service.callback_url,display:'touch',scope:'publish_stream,email' })
      ...
      state.get(tid,function(td){
        ...
        clients[service.name].getOAuthAccessToken(
          parsedurl.query.code , 
            {redirect_uri:service.callback_url,display:'touch'}, 
  ...
  oauthdefs.usermeta.linkedin = function(service,state,clients,ctxt,cb){
    ...
    clients[service.name].get(
      geturl, ctxt.token, ctxt.secret, function(error,data,response){
        ...
        var m_id = /<id>([^<]+)<\/id>/.exec(oneline)
        var m_pp = /<public-profile-url>http:\/\/www.linkedin.com\/in\/([^<]+)<\/public-profile-url>/.exec(oneline)

        if( m_id && m_id[1] ) {
          ctxt.userid = m_id[1]

          if( m_pp && m_pp[1] ) {
            ctxt.username = m_pp[1]
          }
          else {
            ctxt.username = ctxt.userid
          }
    ...
    function login(user,cbl) {
      ...
      self.seneca.act(
        ...
        cberr(cbl,function(out){
          ...
          if( out.pass ) {
            console.log('SENECA USER SET LOGIN COOKIE'+JSON.stringify(out))
      ...
      // new user, register and login
        ...
        userent.load$({nick:username},cberr(cb,function(existing){
          if(!existing) {
            ...
            existing.user[service.name+'_id'] = ctxt.userid
