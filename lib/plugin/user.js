...
function UserPlugin() {
  ...
  */
  ...
  self.init = function(seneca,cb){
    ...
    // args={password}
    seneca.add({on:self.name, cmd:'encrypt_password'},function(args,seneca,cb){
    ...
    seneca.add({on:self.name, cmd:'verify_password'},function(args,seneca,cb){
      var shasum = crypto.createHash('sha1')
    ...
    seneca.add({on:self.name, cmd:'change_password'},function(args,seneca,cb){
      var user = seneca.make(args.tenant,'sys','user')
  ...
  oauthdefs.v1.login = function(service,cache,clients,cb) {
    return function(req,res){
      ...
      console.dir(cache,'v1 request token')
      ...
      clients[service.name].getOAuthRequestToken(
          ...
          additionalParameters) 
        {
          ...
          console.dir(cache[tid],'v1 td request token')
  ...
  oauthdefs.v1.callback = function(service,cache,clients,cb) {
    return function(req,res){
      self.seneca.log('plugin','user','oauth','callback',service.name)
      var ctxt = {req:req,res:res,service:service.name}
      var parsedurl = url.parse(req.url, true)

      console.dir(cache,'access token')

      var cookies = new Cookies( req, res )
      var tid = cookies.get('seneca-oauth')
      var td = cache[tid]
      console.dir(td,'v1 td access token')
  ...
  oauthdefs.v2.login = function(service,cache,clients,cb) {
    return function(req,res){
      if( oauthdefs.redirect(service,req,res) ) return;
      ...
      console.dir(cache[tid],'v2 td request token')
  ...
  oauthdefs.v2.callback = function(service,cache,clients,cb) {
    return function(req,res){
      var ctxt = {req:req,res:res,service:service.name}
      self.seneca.log('plugin','user','oauth','callback',service.name)
      ...
      var tid = cookies.get('seneca-oauth')
      var td = cache[tid]
      console.dir(td,'v2 td access token')
  ...
  oauthdefs.usermeta.twitter = function(service,cache,clients,ctxt,cb) {
  ...
  oauthdefs.usermeta.facebook = function(service,cache,clients,ctxt,cb) {
  ...
  oauthdefs.usermeta.linkedin = function(service,cache,clients,ctxt,cb){
  ...
  oauthdefs.userhandler= function(service,cache,clients,ctxt,cb) {
