...
function prepare(options: any, sourcemap: any, basemodule: any, optionShape: any, initial: any) {
  ...
  if (argv.seneca) {
    ...
    if ('string' === typeof sourcemap.argv.from) {
      sourcemap.argv = Common.deep(
        load_options(sourcemap.argv.from, basemodule),
        sourcemap.argv
      )
    }
...
function load_options(origfrom: any, basemodule: any) {
  var out = {}
