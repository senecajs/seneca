...
function resolve_options(callmodule, defaults, orig_initial) {
    ...
    options = prepare(options, sourcemap, basemodule, optionShape, orig_initial);
    ...
    function set_options(input) {
        ...
        if ('string' === typeof input) {
            options = prepare(options, sourcemap, basemodule, optionShape, input);
        }
...
function prepare(options, sourcemap, basemodule, optionShape, initial) {
    ...
    if ('string' === typeof from) {
        sourcemap.loaded = load_options(from, basemodule);
    }
    // Option debug.argv allows for testing.
    // First two elements are stripped!
    var argv = Minimist(((initial && initial.debug && initial.debug.argv) || process.argv).slice(2));
    ...
    if (argv.seneca) {
        ...
        if ('string' === typeof sourcemap.argv.from) {
            sourcemap.argv = Common.deep(load_options(sourcemap.argv.from, basemodule), sourcemap.argv);
        }
