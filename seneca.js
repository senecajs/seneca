...
function make_seneca (initial_options) {
  ...
  function api_logroute (entry, handler) {
    if (arguments.length === 0) return root.log.router.toString()
  ...
  function parseConfig (args, options) {
    ...
    if (_.isArray(config)) {
      if (config.length === 1 && _.isObject(config[0])) {
        out = config[0]
      } else {
        config[0] && (out.port = parseInt(config[0], 10))
        config[1] && (out.host = config[1])
        config[2] && (out.path = config[2])
      }
    }
  ...
  function api_pin (pattern, pinopts) {
    ...
    if (pinopts && pinopts.include) {
      pinopts.include.forEach(function (methodname) {
  ...
  var pm_custom_args = {
    rules: {
      entity$: function (ctxt, cb) {
        var val = ctxt.point
        if (val.entity$ && val.canon$({isa: ctxt.rule.spec})) {
          return cb()
        }
  ...
  function api_sub () {
    ...
    if (_.every([pattern.in$, pattern.out$, pattern.error$, pattern.cache$, pattern.default$, pattern.client$], function (el) {
      return el == null
    })) {
      pattern.in$ = true
    }
    ...
    if (!private$.handle_sub) {
      private$.handle_sub = function (args, result) {
        if (args.meta$.entry !== true) return
  ...
  function api_add () {
    ...
    var strict_add = (pattern.strict$ && pattern.strict$.add != null) ? !!pattern.strict$.add : !!so.strict.add
    ...
    if (_.keys(pattern_rules).length > 0) {
      ...
      try {
        ...
        try {
         ...
         // handle throws of non-Error values
          if (!util.isError(ex)) {
            _err = new Error(_.isObject(ex) ? jsonic.stringify(ex) : ('' + ex))
          }

          callback_error(instance, _err, actmeta, result, cb,
          actend - actstart, callargs, prior_ctxt, act_callpoint)
        }
      } catch (ex) {
  ...
  function act_error (instance, err, actmeta, result, cb,
    ...
    if (!err.seneca) {
      ...
      result[0] = err
    } else if (err.orig && _.isString(err.orig.code) &&
        ...
        fn = function (data, done) {
          ...
          if (args.strargs) {
            _.each(actargs, function (v, k) {
              var $ = data // needed inside the eval
...
function makedie (instance, ctxt) {
  ...
  var die = function (err) {
    ...
    try {
      ...
      var fatalmodemsg = instance.fixedargs.fatal$ ? fatalError : ''
...
function make_legacy_fail (so) {
  return function () {
    ...
    var cb = _.isFunction(arguments[arguments.length - 1]) ? arguments[arguments.length - 1] : null
